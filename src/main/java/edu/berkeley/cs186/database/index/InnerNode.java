...
class InnerNode extends BPlusNode {
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        BPlusNode LeftNode = getChild(0);
        return LeftNode.getLeftmostLeaf();
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        BPlusNode childNode = getChild(insertIndex);
        Optional<Pair<DataBox, Long>> splitPair = childNode.put(key, rid);
        if (splitPair.isPresent()){
            keys.add(insertIndex, splitPair.get().getFirst());
            children.add(insertIndex+1, splitPair.get().getSecond());
            if (keys.size() > metadata.getOrder() * 2){
                ...
                splitPageNum = rightInnerNode.getPage().getPageNum();
            }
        }
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (children == null || children.isEmpty()) {
            throw new IllegalStateException("InnerNode has no children");
        }

        int d = metadata.getOrder();
        int maxKeySize = 2 * d; // 4
        Optional<Pair<DataBox, Long>> splitPair = Optional.empty();

        BPlusNode lastChild = getChild(children.size() - 1);
        while (data.hasNext()) {
            splitPair = lastChild.bulkLoad(data, fillFactor);
            ...
            if (splitPair.isPresent()) {
                ...
                if (this.keys.size() > maxKeySize) {
                    ...
                    List<DataBox> rightKeys = new ArrayList<>(this.keys.subList(splitPos + 1, this.keys.size()));
                    List<Long> rightChildren = new ArrayList<>(this.children.subList(splitPos + 1, this.children.size()));
                    this.keys.subList(splitPos, this.keys.size()).clear();
                    this.children.subList(splitPos + 1, this.children.size()).clear();

                    InnerNode rightNode = new InnerNode(metadata, bufferManager, rightKeys, rightChildren, treeContext);
                    rightNode.sync();
                    this.sync();

                    return Optional.of(new Pair<>(popUpKey, rightNode.getPage().getPageNum()));
                }

                lastChild = BPlusNode.fromBytes(metadata, bufferManager, treeContext, newChildPageNum);
            }
        }
